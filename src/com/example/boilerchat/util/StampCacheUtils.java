...
*/
	public static File copyToCache(Uri source, int maxSize) throws IOException {
		InputStream is = BoilerChatApplication.getContext().getContentResolver().openInputStream(source);
		...
		try {
			...
			is = BoilerChatApplication.getContext().getContentResolver().openInputStream(source);
			Bitmap bitmap = null;
	...
	*/
	public static File getCacheFile(String uri) {
		...
		getExternalCacheDir(BoilerChatApplication.getContext()).getPath() : BoilerChatApplication.getContext().getCacheDir().getPath());
		if (cacheDir.exists()) {
			cacheDir.mkdirs();
		}
